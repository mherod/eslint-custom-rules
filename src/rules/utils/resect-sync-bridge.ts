...
export function decodeBridgeResponse<TResult>(
  responseText: string
  ...
  try {
    ...
    // Fall through to the invalid-response result below.
  }

  return {
    error: "Bridge worker returned an invalid response",
    handled: false,
    ok: false,
    result: null,
  };
...
export function writeBridgeDebug(
  request: BridgeDebugRequest,
  error: string
): void {
  if (process.env.RESECT_BRIDGE_DEBUG !== "1") {
    return;
  }

  const operation = request.operation.replace(/[\r\n]/gu, " ");
  const filePath = request.filePath.replace(/[\r\n]/gu, " ");
  const safeError = error.replace(/[\r\n]/gu, " ");
  process.stderr.write(
    `[eslint-plugin-custom/resect-bridge] operation=${operation} file=${filePath} error=${safeError}\n`
  );
}
...
function parseSourceFileFromDisk(filePath, sourceFileCache) {
  if (sourceFileCache && sourceFileCache.has(filePath)) {
...
// Memoized entry point: one owner lookup per (modulePath, exportName) pair
// within a single direct-import request. The shared sourceFileCache keeps each
// unique module read/parsed at most once per request; both caches are
// request-local so no stale cross-request state is introduced.
function resolveExportOwnerCached(
  modulePath,
  exportName,
  project,
  resect,
  ...
  const ownerPath = resolveExportOwner(
    ...
    project,
    resect,
    requestCaches.sourceFiles,
    new Set()
  );
  requestCaches.owners.set(cacheKey, ownerPath);
  return ownerPath;
}
...
function resolveExportOwner(
  ...
  const sourceFile = parseSourceFileFromDisk(modulePath, sourceFileCache);
  if (!sourceFile) {
    return null;
  }

  // resect >=1.8.0 includes re-exported names ("export { X } from ...") in
  // scanExports with no marker distinguishing them from local declarations, so
  // names surfaced by scanBarrelExports must be excluded before treating a
  // scanExports hit as proof of local ownership.
  ...
  for (const barrel of barrelExports) {
    for (const entry of barrel.exports) {
      if (entry.type === "named") {
        reExportedNames.add(entry.alias ?? entry.name);
      }
    }
  }
  ...
  if (
    localExports.some(
      (entry) =>
        ...
        entry.type !== "default" &&
        !entry.isType &&
        !reExportedNames.has(entry.name)
    )
  ) {
    return modulePath;
  }
...
async function resolveDirectImports(payload) {
  ...
  return payload.bindings
    .map((binding) => {
      const ownerPath = resolveExportOwnerCached(
        resolved.path,
...
async function classifyBarrelImport(payload) {
  const projectContext = await getProjectContext(payload.filePath);
  if (!projectContext) {
    return null;
  }
  ...
  const sourceFile = parseSourceFileFromDisk(resolved.path);
  if (!sourceFile) {
    return null;
  }

  return resect.scanBarrelExports(sourceFile, project).length > 0;
}
...
function runBridgeWorkerRequest<TResult>(
  ...
  if (
    ...
    return {
      error: "Bridge worker is unavailable or busy",
      handled: false,
      ok: false,
      result: null,
    };
  ...
  if (encodedRequest.byteLength > state.requestBuffer.byteLength) {
    return {
      error: "Bridge request exceeds the shared buffer capacity",
      handled: false,
      ok: false,
      result: null,
    };
  ...
  if (waitResult === "timed-out") {
    ...
    return {
      error: "Bridge worker request timed out",
      handled: false,
      ok: false,
      result: null,
    };
  ...
  if (Atomics.load(state.control, BRIDGE_STATE_INDEX) !== STATE_RESPONSE) {
    disableBridgeWorker(state);
    return {
      error: "Bridge worker entered an unexpected state",
      handled: false,
      ok: false,
      result: null,
    };
  ...
  if (responseText === null) {
    disableBridgeWorker(state);
    return {
      error: "Bridge worker response length is invalid",
      handled: false,
      ok: false,
      result: null,
    };
  ...
  const result = decodeBridgeResponse<TResult>(responseText);
  if (!result.handled) {
    disableBridgeWorker(state);
  }
  return result;
...
function runBridgeRequest<TResult>(request: SyncBridgeRequest): TResult | null {
  ...
  writeBridgeDebug(
    request,
    workerResult.error ?? "Bridge worker request failed"
  );
...
export function classifyBarrelImportSync(request: {
  filePath: string;
  specifier: string;
}): boolean | null {
